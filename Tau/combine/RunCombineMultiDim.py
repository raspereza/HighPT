...
if __name__ == "__main__":
    ...
    check_file_existence("taunu_{}_lowpt_{}.txt".format(name, args.era))
    check_file_existence("taunu_{}_highpt_{}.txt".format(name, args.era))

    # Combine W*->mu+v and W*->tau+v cards
    subprocess.call(["combineCards.py", "munu_{}_{}.txt".format(name, args.era), "taunu_{}_lowpt_{}.txt".format(name, args.era), "taunu_{}_highpt_{}.txt".format(name, args.era)], stdout=open("tauID_{}_ptbinned.txt".format(name), 'w'))

    # Creating workspace
    subprocess.call(["combineTool.py", "-M", "T2W", "-P", "HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel", "--PO", '"map=^.*/*_highpt_{}:r_highpt[1,0,2]"'.format(args.era), "--PO", '"map=^.*/*_lowpt_{}:r_lowpt[1,0,2]"'.format(args.era), "-o", "tauID_{}_ptbinned.root".format(name), "-i", "tauID_{}_ptbinned.txt".format(name)])

    # Doing fit
    subprocess.call(["combineTool.py", "-M", "FitDiagnostics", "--saveNormalizations", "--saveShapes", "--saveWithUncertainties", "--saveNLL", "--redefineSignalPOIs", "r_lowpt,r_highpt", "--robustFit", "1", "-m", "200", "-d", "tauID_{}_ptbinned.root".format(name), "--cminDefaultMinimizerTolerance", "0.1", "--cminDefaultMinimizerStrategy", "1", "-v", "2"])

    # Renaming output
    os.rename("fitDiagnostics.Test.root", "tauID_{}_ptbinned_fit.root".format(name))
